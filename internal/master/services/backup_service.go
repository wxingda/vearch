...
func (s *BackupService) backupSchema(ctx context.Context, dbName, spaceName string, backup *entity.BackupSpace) (err error) {
	mc := s.client.Master()
	...
	objectName := fmt.Sprintf("%s/%s/%s/%s.schema", config.Conf().Global.Name, dbName, space.Name, space.Name)
...
func (s *BackupService) restoreSchema(ctx context.Context, dbService *DBService, spaceService *SpaceService, configService *ConfigService, dbName, spaceName string, backup *entity.BackupSpace) (err error) {
	mc := s.client.Master()
	dbID, err := mc.QueryDBName2Id(ctx, dbName)
	if err != nil {
		return err
	...
	bucketName := backup.S3Param.BucketName
	objectName := fmt.Sprintf("%s/%s/%s/%s.schema", config.Conf().Global.Name, dbName, spaceName, spaceName)
...
func (s *BackupService) BackupSpace(ctx context.Context, dbService *DBService, spaceService *SpaceService, configService *ConfigService, dbName, spaceName string, backup *entity.BackupSpace) (err error) {
	if backup.Command == "create" || backup.Command == "export" {
		err = s.backupSchema(ctx, dbName, spaceName, backup)
		if err != nil {
			log.Error(err)
			return err
		}
	} else if backup.Command == "restore" {
		err = s.restoreSchema(ctx, dbService, spaceService, configService, dbName, spaceName, backup)
		if err != nil {
			log.Error(err)
			return err
		}
