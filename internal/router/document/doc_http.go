...
func (handler *DocumentHandler) proxyMaster(group *gin.RouterGroup) error {
	...
	group.POST(fmt.Sprintf("/dbs/:%s/backup", URLParamDbName), handler.handleMasterRequest)
	group.POST(fmt.Sprintf("/dbs/:%s/spaces/:%s/backup", URLParamDbName, URLParamSpaceName), handler.handleMasterRequest)
	group.POST(fmt.Sprintf("/dbs/:%s/spaces/:%s/export", URLParamDbName, URLParamSpaceName), handler.handleMasterRequest)
	// space handler
